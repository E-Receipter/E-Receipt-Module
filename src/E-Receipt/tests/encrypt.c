...
char SOURCE[] = "compressed.txt";
char KEY[] = "key.txt";
char DEST[] = "encrypt.txt";
...
int main(){
    ...
    jab_data* encrypted_data = RG_encrypt(key,data);
    FILE* dest_file = fopen(DEST,"wb");
    fwrite(encrypted_data->data,1,encrypted_data->length,dest_file);
    fclose(dest_file);
    free(data);
    free(key);
    free(encrypted_data);
